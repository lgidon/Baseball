...
def test_admin_get_authenticated(mock_start_worker, client):
    with captured_templates(flask_app) as templates:
        ...
        # The actual SETTINGS may contain extra keys (like admin_user, admin_password).
        # Check only the keys that were overridden by the patch.
        settings = context['settings']
        assert settings['user_name'] == "Baseball Fan"
        assert settings['theme_color'] == "dark"
        assert settings['sync_interval_mins'] == 5
